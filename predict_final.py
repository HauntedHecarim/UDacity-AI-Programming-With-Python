...
def load_checkpoint(checkpoint_path):
    checkpoint = torch.load("chekpoint.pth")
...
def predict(image_tensor, model, device, cat_to_name, top_k):
    model.eval()
    with torch.no_grad():
        image_tensor = torch.FloatTensor(image_tensor).unsqueeze(0).to(device)  # Move input tensor to device
        model = model.to(device)  # Move model to device
        output = model(image_tensor)
        probs, indices = torch.topk(torch.nn.functional.softmax(output, dim=1), top_k)
    top_probs = probs.squeeze().cpu().numpy()
    top_labels = indices.squeeze().cpu().numpy()
